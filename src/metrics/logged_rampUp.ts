    ...
    function traverse(node: Node) {
      switch (node.type) {
        case 'heading':
          ...
          debug(`Found heading: ${title}`); // Changed from console.log to debug
        ...
        case 'code':
          ...
          debug(`Code block found. Total code blocks: ${metrics.codeBlocks}`);
          break;
  
        case 'link':
          metrics.links += 1;
          debug(`Link found. Total links: ${metrics.links}`);
          break;
...
export function calculateRampUpScore(metrics: Metrics): number {
  ...
  debug(`Essential sections score: ${essentialScore.toFixed(2)}`);
  ...
  debug(`Code blocks score: ${codeBlocksScore.toFixed(2)}`);
  debug(`Links score: ${linksScore.toFixed(2)}`);

  //Calculate and return arithmetic mean score [0, 1]
  const finalScore = (essentialScore + codeBlocksScore + linksScore) / 3;
  info(`Final Ramp-Up Score: ${finalScore.toFixed(2)}`);
  return finalScore;
}
