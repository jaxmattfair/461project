...
const fetchAllClosedIssues = async (owner: string, repo: string): Promise<any[]> => {
    let issues: any[] = [];
    let page = 1;
    const perPage = 100;
    ...
    while (true) {
        try {
            ...
            const onlyIssues = data.filter((issue: any) => !issue.pull_request);
            issues = issues.concat(onlyIssues);

            if (data.length < perPage) break;
            page += 1;
        } catch (err) {
            error(`Error fetching closed issues for page ${page}: ${(err as Error).message}`);
            break;
        }
    }
...
const fetchAllClosedPullRequests = async (owner: string, repo: string): Promise<any[]> => {
    ...
    while (true) {
        try {
            ...
            error(`Error fetching closed pull requests for page ${page}: ${(err as Error).message}`);
            break;
        }
    }
...
const computeAverageResponseTime = (responseTimes: number[]): number => {
...
const normalizeResponseTime = (avgResponseTime: number): number => {
...
export const calculateResponsiveness = async (repoURL: string): Promise<number> => {
    try {
        ...
        info(`Fetched ${issues.length} closed issues.`);
        info(`Fetched ${pullRequests.length} closed pull requests.`);
        ...
        // Normalize to [0,1]
        const responsiveScore: number = normalizeResponseTime(overallAvgResponseTime);
